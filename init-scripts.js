...
function createArticle(articlejson, endpoint, currentissue) {
	...
	article.authorimgurl = metadata.authorimg; // json.images.s3[metadata.author.img].url;
	...
	Config.model
		...
		.then(config => {
			...
			fetch(
				...
				.catch(err => {
					console.error(
						"Error retrieving list of endpoints from https://kerckhoff.dailybruin.com/api/packages/prime?endpoints=true"
					);
					//done(err);
				});
		});
