...
Article.add({
	...
	featured: {
		type: Types.Select,
		options: "no, featured, main feature",
		default: "no",
		note: "Is this a featured article in its section?"
	},
		...
		.then(response => {
			response
				...
				.then(json => {
					...
					this.authorimgurl = metadata.author
						? json.images.s3[metadata.authorimg].url
						: "";
					this.authorbio = metadata.author ? metadata.authorbio : "";
